...
**触发问题**:在给 role/menu/dept 路由挂 audit 声明时，`audit: { module: "dept", action: "create" }` 写法跟 `perm: ["sys:dept:create"]` 风格不一致，问为什么不用 `audit: "dept:create"`。
...
1. **OR 权限场景**：某些接口可以设 `perm: ["sys:user:list", "sys:role:assign"]`，两个权限任一个有就行。如果用 `string`，遇到 OR 场景得改类型、改内部逻辑、改所有调用处。
