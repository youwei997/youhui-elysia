...
按这个思路，`requireRole` 根本没必要做，有 `perm` 就够了——用户拿到角色，角色有菜单，菜单有 perm，前端按钮控制到 perm，接口控制到 perm，完事。
...
如果只有 `perm`：那框架级操作（如"查看在线用户""强制踢人"）没有对应按钮/菜单，怎么控？
...
但现实中没用。如果 ADMIN 有权限点，那直接声明 `perm` 就够了。如果 ADMIN 没有权限点（那它不应该能访问）。同时声明说明权限设计有问题，不是 macro 的问题。
