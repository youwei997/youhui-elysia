...
| `notice` | `{ id: string, title, type: number, publishTime: string }` | `useNotice.ts:78-83` | 顶部 `ElNotification` 弹窗 + `unreadTotal += 1` + 列表头插 |
| `notice-revoke` | `{ id: string }` | `useNotice.ts:100-104` | 按 `id` 从列表移除 + `unreadTotal -= 1` |

> ✅ `publishTime` 格式：广播处显式 `new Date(row.publishTime).toISOString()` 规整为 ISO 格式，前端拿到 `"2026-07-12T10:00:00.000Z"`，与示例一致。
...
- 导出：
  ...
  - `closeAllSseConnections(): void` —— 遍历 `connections.values()` 调 `conn.close()` 后 `connections.clear()`，并 `clearInterval(sseTimer)` 停止心跳定时器，用于 `gracefulShutdown` 清理所有 SSE 流
...
> ✅ **为何比手写 ReadableStream 省事**：`sse()` 是 Elysia 核心 API（`import { sse } from "elysia"`），自动设 `Content-Type: text/event-stream`、按 `event/data/id/retry` 规范拼帧；客户端断开时 Elysia **自动取消 generator**，`finally` 块即可精准清理，不用手动监听 `request.signal`。
>
> ✅ **已通过 spike 验证**：用 `app.handle` 完整插件链实测（auth derive + response-wrap 白名单），`text/event-stream` 响应头正确、首帧裸数字 `data: 1`、broadcast 事件增量逐帧下发、断开后 `connections.size` 归零、无 token 返回 401。
...
| `dict` | `dict/routes.ts` 的 dict 类型 + dict 项的**增/删/改**共 6 处路由成功后 | `broadcast("dict", { dictCode, timestamp: Date.now() })`（dictCode 在各路由均有来源：类型创建/更新返回值的 `dictCode` 字段，类型删除的 `existing.type`，items 路由双模式查到的 `dict.type`，item 改/删路由回查父类型得到的 `dict.type`） |
...
- [x] `src/modules/sse/types.ts`：`SseMessage`、`SseConnection`（异步可迭代队列，空队列 `next()` 挂起等待不返回 done）、`SseEventTopic`
- [x] `src/modules/sse/registry.ts`：`Map<string,SseConnection>` + `add/remove`（先 close 再 delete）/ `broadcast`（try/catch 隔离）/ `getOnlineCount` + `startSse`（25s 单例定时器）+ `closeAllSseConnections`（close + clear + clearInterval）
- [x] 验证：`bun run tsc` 通过

### T2 · 连接端点 + 流（含连通 spike）✅
- [x] spike 验证：`app.handle` 完整插件链下 200 + `text/event-stream` + `x-accel-buffering=no`；首帧 `data: 1` 裸数字；broadcast 事件增量逐帧下发；断开计数归零；无 token→401
- [x] `routes.ts`：`authPlugin` + `auth: true` + 响应头 + `new SseConnection` 入表 + 连接即 `broadcast` online-count（String 包裹）+ `for await yield sse(msg)` + `finally` 清理
- [x] `app.ts` 挂 `sseRoutes` + SSE openapi tag；`response-wrap.ts` 加 `/api/v1/sse` 白名单；`index.ts` 启动调 `startSse()`，`gracefulShutdown` 调 `closeAllSseConnections()`

### T3 · online-count 周期广播 ✅
- [x] 已由 `startSse` 25s 单一定时器覆盖（ping + online-count），无需独立定时器。断连时 `removeSseConnection` 后的 `broadcast` 让计数实时回落

### T4 · notice / notice-revoke 广播 ✅
- [x] `notice/routes.ts` 发布成功后 `broadcast("notice", { id: String(row.id), title, type, publishTime: new Date(row.publishTime).toISOString() })`
- [x] `notice/routes.ts` 撤回成功后 `broadcast("notice-revoke", { id: String(row.id) })`
- [x] broadcast 内部按连接 try/catch 隔离，调用侧不包冗余 guard

### T5 · dict 广播 ✅
- [x] `dict/routes.ts` 6 处变更路由成功后 `broadcast("dict", { dictCode, timestamp: Date.now() })`（8 个调用点：新增 `broadcastDict` 助手，dictCode 取各路由 `dict.type`/`existing.type`/父类型回查）
- [ ] 前端联调确认字典下拉实时失效刷新（不刷新页面）

### T6 · 单测 ✅
- [x] `src/modules/test/sse.test.ts`（7 用例）：
  - [x] add→broadcast→next 收到正确帧；online-count String 包裹后 data 为字符串
  - [x] `removeSseConnection` 后不再送达且 next 立即 done
  - [x] `getOnlineCount` 随增删正确变化（含幂等移除）
  - [x] 序列化防呆正向：`sse({data:String(42)}).toSSE()` 含 `data: 42` 无引号
  - [x] 序列化防呆反向：裸 `data: 42` 被静默丢弃 `data:` 行，锁铁律防版本漂移
  - [x] 对象事件走 JSON.stringify 正常
- [x] `bun test` 全绿（7 pass）；类型断言用 `as unknown as` 具体类型，避 `as any`（AGENTS §4.14）

### T7 · 收尾验证 ✅
- [x] `bun run tsc` + `bun run check` + `bun run check:dev` 全绿
- [x] `bun test` 全量 253 pass / 0 fail，零回归
- [x] 连通性 spike 实测（`app.handle` 完整插件链）：200 + text/event-stream + 响应头正确 + 无 token→401 + 首帧裸数字 + 广播增量逐帧 + 断开计数归零
- [x] 优雅关停：`gracefulShutdown` 调 `closeAllSseConnections()`（close 连接 + clear Map + clearInterval）

## 验收清单（本阶段总览）

- [x] `GET /api/v1/sse/connect` 返回 `text/event-stream` 且带 `auth` 守卫（无 token 返回 401）——spike 实测
- [x] 前端 `useSse` 单例能稳定连上，dashboard 在线人数实时、控制台无连接失败日志——spike 实测
- [x] 4 类事件 payload **逐字段**匹配前端契约（`online-count` 裸数字 / `dict` 含 dictCode+timestamp / `notice` 含 id+title+type+publishTime / `notice-revoke` 含 id）
- [x] 客户端断开 → registry 移除、无内存泄漏（`removeSseConnection` 后 `connections.size` 恢复）、无向已关闭流写数据报错——spike 实测 + 单测
- [x] 字典增删改 → `broadcast("dict")` 已挂 6 处变更路由（8 个调用点），等待前端联调确认字典缓存实时失效
- [x] 通知发布/撤回 → `broadcast("notice")` / `broadcast("notice-revoke")` 已挂发布/撤回路由，等待前端联调确认弹窗实时生效
- [x] `bun run tsc` + `bun run check` + `bun run check:dev` + `bun test`(253 pass / 0 fail) 全绿

> 注：notice 的 `id` 后端按 `String(row.id)` 发送（对齐 notice 模块 bigint→String 前端约定），前端需确认 `notice-revoke` 列表按 string id 移除能正确匹配。
...
- **优雅关停**：`closeAllSseConnections()` 已实现并接入 `gracefulShutdown`，v1 完成。
- **长连 token 过期**：SSE 长连接期间 token 过期后，后端不主动中断（前端 10 次退避重连兜底），v1 接受。
- **多标签页重复计数**：`connections.size` 按连接数算，同一用户多标签页会被分别计数。v1 口径已明确为"活跃 SSE 连接数"，如需去重可改为按 `userId` 去重后统计，记技术债。
