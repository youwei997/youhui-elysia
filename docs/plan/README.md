...
[2026-06-17] 阶段 3.8 完成 + 阶段 3 全部完成。user 5 个路由挂 auth: true，OpenAPI 接入 bearerAuth 安全方案（login/refresh 显式覆盖为公开）。biome/typecheck/test 全过；运行时验证（curl + Swagger UI）由开发者手动完成。进入阶段 4 RBAC。
```
