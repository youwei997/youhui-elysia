...
| 11 · 多租户 | ⭐⭐⭐ | 4-5d | ✅ 已完成 | [stage-11-multi-tenant.md](./stage-11-multi-tenant.md) |
...
[2026-07-15] 阶段 11 多租户完成。单库多租户架构落地：tenantId 写入 JWT + switch-tenant 重签 + tenantPlugin derive + tenantEq helper；全部 8 个业务模块 query 接入 tenantId 隔离（user/role/dept/menu/notice/oper-log/storage + buildDataScopeContext）；tenant 模块 11 端点 CRUD + tenant-plan 模块 8 端点 CRUD；seed 脚本补充 tenantId + 菜单 scope 分类 + sys_tenant_menu/sys_tenant_plan_menu 数据。review 修复：JWT 增加 homeTenantId 字段（解决切换租户后 refresh token 权限丢失）、findMenusByRoleCodes 增加套餐级二次过滤、新增 canAccessTenant 服务层方法、audit-log plugin 显式写入 tenant_id、role/dept 唯一约束改为租户内复合唯一（uniqueIndex + where 软删兼容）、跨租户隔离单测 27 场景全绿。全量 316 单测 PASS。
```
